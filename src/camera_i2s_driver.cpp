...
static esp_err_t i2s_camera_config(const camera_config_t *config) {
    ...
    uint64_t pin_mask = 
        (1ULL << config->pin_d1) | (1ULL << config->pin_d2) |
...
esp_err_t camera_i2s_init(const camera_config_t *config) {
    ...
    ESP_LOGI(TAG, "SCCB/I2C initialized (SDA=%d, SCL=%d)", config->pin_sccb_sda, config->pin_sccb_scl);
    ...
    ESP_LOGI(TAG, "  DMA buffers: %d x %d bytes", config->dma_buffer_count, config->dma_buffer_size);

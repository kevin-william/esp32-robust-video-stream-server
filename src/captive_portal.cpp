...
bool connectToWiFiWithStaticIP(const char* ssid, const char* password, IPAddress ip,
                               IPAddress gateway, int timeout_ms) {
...
bool tryConnectSavedNetworks() {
    ...
    for (int i = 0; i < g_config.network_count; i++) {
        Serial.printf("Trying saved network %d/%d: %s (priority %d)\n", i + 1,
                      g_config.network_count, g_config.networks[i].ssid,
        ...
        if (g_config.networks[i].use_static_ip) {
            ...
            connected = connectToWiFiWithStaticIP(
                g_config.networks[i].ssid, g_config.networks[i].password, ip, gateway, 15000);
        } else {
            connected =
                connectToWiFi(g_config.networks[i].ssid, g_config.networks[i].password, 15000);
        }

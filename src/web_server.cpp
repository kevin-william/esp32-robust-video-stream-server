...
void initWebServer() {
    ...
    server.on("/", HTTP_GET, [](AsyncWebServerRequest *request) {
        ...
        html +=
            ...
            "auto;background:white;padding:20px;border-radius:10px;box-shadow:0 2px 5px "
            "rgba(0,0,0,0.1)}";
        ...
        html +=
            ...
            "20px;border-radius:5px;cursor:pointer;margin:5px}";
        html += "button:hover{background:#0056b3}";
        html +=
            "input,select{width:100%;padding:8px;margin:5px 0;border:1px solid "
        ...
        if (ap_mode_active) {
            html +=
                "<div class='status'>Configuration Mode - Connect your WiFi network below</div>";
            html += "<h2>WiFi Setup</h2>";
            html +=
                "<p><strong>Note:</strong> The device will remain in this mode until successfully "
                "connected.</p>";
            ...
            html +=
                "<p style='margin:10px 0;color:#666'><strong>Optional:</strong> Static IP "
                "Configuration (leave blank for DHCP)</p>";
            html +=
                "<input type='text' id='static_ip' placeholder='Static IP (e.g., 192.168.1.100)'>";
            html += "<input type='text' id='gateway' placeholder='Gateway (e.g., 192.168.1.1)'>";
            html +=
                "<button onclick='connectWiFi()' "
                "style='margin-top:10px;width:100%;padding:12px;font-size:16px'>Connect to "
                "WiFi</button>";
            ...
            html += "<h2>System</h2>";
            html +=
                "<button onclick='restart()' style='background:#ff9800'>Restart Device</button>";
            html +=
                "<button onclick='factoryReset()' style='background:#f44336'>Factory "
                "Reset</button>";
            html +=
                "<p style='color:#666;font-size:12px'>Factory Reset will erase all WiFi networks "
                "and return to setup mode</p>";
            html += "<h2>Settings</h2>";
            html +=
                "<label>Quality (0-63): <input type='range' id='quality' min='0' max='63' value='" +
                String(g_config.camera.quality) +
                "' onchange='setControl(\"quality\",this.value)'></label>";
            html +=
                "<label>Brightness: <input type='range' id='brightness' min='-2' max='2' value='" +
                String(g_config.camera.brightness) +
                "' onchange='setControl(\"brightness\",this.value)'></label>";
            html += "<label>LED Intensity: <input type='range' id='led' min='0' max='255' value='" +
                    String(g_config.camera.led_intensity) +
                    "' onchange='setControl(\"led_intensity\",this.value)'></label>";
        }

        html += "</div><script>";
        html +=
            "function connectWiFi(){const s=document.getElementById('ssid').value;const "
            "p=document.getElementById('password').value;if(!s||!p){alert('SSID and Password are "
            "required!');return;}const "
            "msg=document.getElementById('status-message');msg.innerHTML='<p "
            "style=\"color:#007bff\">Connecting to WiFi...</p>';const "
            "data={ssid:s,password:p};const "
            "sip=document.getElementById('static_ip').value;if(sip){const "
            "gw=document.getElementById('gateway').value;if(gw){data.use_static_ip=true;data."
            "static_ip=sip;data.gateway=gw;}}fetch('/"
            "wifi-connect',{method:'POST',headers:{'Content-Type':'application/"
            "json'},body:JSON.stringify(data)}).then(r=>r.json()).then(d=>{msg.innerHTML='<p "
            "style=\"color:'+(d.success?'green':'red')+';font-size:16px\">'+d.message+'</"
            "p>';}).catch(e=>{msg.innerHTML='<p style=\"color:red\">Request failed. Check "
            "credentials and try again.</p>'})}";
        html +=
            "function capture(){fetch('/capture').then(r=>r.blob()).then(b=>{const "
            "url=URL.createObjectURL(b);document.getElementById('image').innerHTML='<img "
            "src=\"'+url+'\">';});}";
        html +=
            "function "
            "setControl(v,val){fetch('/"
            "control?var='+v+'&val='+val).then(r=>r.json()).then(d=>console.log(d));}";
        ...
        html +=
            "function restart(){if(confirm('Restart "
            "device?')){fetch('/restart').then(r=>r.json()).then(d=>{alert('Device restarting... "
            "Please wait 30 seconds.');setTimeout(()=>location.reload(),30000);});}}";
        html +=
            "function factoryReset(){if(confirm('WARNING: This will erase ALL WiFi configurations "
            "and return to setup mode.\\n\\nAre you sure?')){if(confirm('This action cannot be "
            "undone. "
            "Continue?')){fetch('/"
            "factory-reset').then(r=>r.json()).then(d=>{alert(d.message);setTimeout(()=>location."
            "href='http://192.168.4.1',5000);}).catch(e=>alert('Reset initiated'));}}}";
...
void handleCapture(AsyncWebServerRequest *request) {
    ...
    if (!camera_initialized || camera_sleeping) {
        ...
        AsyncWebServerResponse *response = request->beginResponse(
            503, "application/json", "{\"error\":\"Camera is sleeping or not initialized\"}");
        addCORSHeaders(response);
        request->send(response);
        return;
    }
    ...
    if (!fb) {
        ...
        AsyncWebServerResponse *response = request->beginResponse(
            500, "application/json", "{\"error\":\"Failed to capture frame\"}");
        addCORSHeaders(response);
        request->send(response);
        return;
    }
...
void handleControl(AsyncWebServerRequest *request) {
    ...
    String response =
        (res == ESP_OK) ? "{\"success\":true}" : "{\"error\":\"Failed to set " + var + "\"}";
...
void handleFactoryReset(AsyncWebServerRequest *request) {
    ...
    AsyncWebServerResponse *response =
        request->beginResponse(200, "application/json",
                               "{\"success\":true,\"message\":\"Configuration reset. Device "
...
void handleWiFiConnect(AsyncWebServerRequest *request, uint8_t *data, size_t len, size_t index,
    ...
    if (index + len == total) {
        ...
        if (error) {
            AsyncWebServerResponse *response = request->beginResponse(
                400, "application/json", "{\"success\":false,\"message\":\"Invalid JSON\"}");
            addCORSHeaders(response);
            request->send(response);
            return;
        }
        ...
        if (connected) {
            ...
            AsyncWebServerResponse *response =
                request->beginResponse(200, "application/json",
                                       "{\"success\":false,\"message\":\"Unable to connect. Check "
                                       "SSID, password, and signal strength.\"}");
            addCORSHeaders(response);
            request->send(response);
        }
    }
...
void handleStop(AsyncWebServerRequest *request) {
    ...
    AsyncWebServerResponse *response =
        request->beginResponse(200, "application/json",
                               "{\"success\":true,\"message\":\"Camera service stopped and sensor "
                               "in power-down mode. WiFi remains active.\"}");
    addCORSHeaders(response);
    request->send(response);
}
...
void handleStart(AsyncWebServerRequest *request) {
    ...
    if (success) {
        ...
        AsyncWebServerResponse *response = request->beginResponse(
            ...
            "{\"error\":\"Failed to start camera service\",\"message\":\"Check camera connections "
            "and power supply\"}");
        addCORSHeaders(response);
        request->send(response);
    }
